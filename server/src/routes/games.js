    ...
    if (scenario.public.status !== 'available') {
        return res.status(409).json({
            error: {
                ...
                message: 'The requested scenario is not currently available.'
            }
        });
    }
...
router.get('/:gameId', (req, res) => {
    ...
    if (!session) {
        return res.status(404).json({
            error: {
                code: 'GAME_NOT_FOUND',
                message:
                    'The requested game session does not exist.'
            }
        });
    }
    ...
    if (!scenario) {
        return res.status(500).json({
            error: {
                code: 'SCENARIO_CONFIGURATION_MISSING',
                message:
                    'The scenario configuration is unavailable.'
            }
        });
    }
...
router.post('/:gameId/messages', async (req, res) => {
    const session = getGameSession(req.params.gameId);
    ...
    const expiredOutcome = resolveGameOutcome(
        scenario,
        session
    );
    ...
    if (expiredOutcome) {
        ...
        return res.status(409).json({
            error: {
                code: 'GAME_NOT_ACTIVE',
                message:
                    'This negotiation session has ended.'
            },
            data: toPublicGameSession(
                session,
                scenario
    ...
    if (session.status !== 'active') {
        return res.status(409).json({
            error: {
                code: 'GAME_NOT_ACTIVE',
                message:
                    'This negotiation session is no longer active.'
            }
        });
    }
    ...
    if (session.isProcessing) {
        return res.status(409).json({
            error: {
                ...
                message:
                    'A previous transmission is still being processed.'
            }
        });
    }
    ...
    if (!content) {
        return res.status(400).json({
            error: {
                ...
                message:
                    'A transmission message is required.'
            }
        });
    }
    ...
    if (content.length > 800) {
        return res.status(400).json({
            error: {
                ...
                message:
                    'The transmission cannot exceed 800 characters.'
            }
        });
    }
    ...
    if (evidenceId) {
        attachedEvidence = session.evidence.find(
        ...
        if (!attachedEvidence) {
            return res.status(400).json({
                error: {
                    ...
                    message:
                        'The selected evidence does not exist.'
                }
            });
        }
    ...
    try {
        ...
        const turnResult = await evaluateTurn({
            scenario,
            session,
            content,
            attachedEvidence
        ...
        console.dir({
            content,
            revealedFactIds:
                turnResult.evaluation.revealedFactIds,
            fullEvaluation:
        ...
        if (attachedEvidence) {
            attachedEvidence.submitted = true;
        }
        ...
        console.error(
            'Unable to evaluate negotiation turn:',
            error
        );
        ...
        return res.status(502).json({
            error: {
                ...
                message:
                    'The intelligence failed to process the transmission. Please try again.'
            }
        });
    }
});
